...
description: >-
  ...
  minimax-h3 (SeetaCloud ComfyUI) hands off a prompt brief after choosing a
  workflow. Do not SSH, pick ComfyUI workflows, or submit GPU jobs.
---
...
1. If the caller passed an **H3 prompt brief**, read `references/comfy-handoff.md`, lock `prompt_mode` from that brief, and apply its override table. Do not re-guess the mode from the images alone.
...
- Match the total duration of the description to the requested video length (4–15 seconds unless a brief specifies otherwise).
- Keep reference labels consistent (e.g. `<Picture 1>`, `<Video 1>`, `<Audio 1>`) across every section.
- Prefer concrete visual and audio details over empty praise such as "beautiful". Style tokens at the start of Shot 1 (`live-action`, `cinematic`, `2D-animated`) are allowed.
