...
description: >-
  ...
  the caller hands off a structured handoff (a locked director card from
  h3-short-drama-director, or a minimax-h3 brief) after choosing a workflow.
...
1. If the caller passed a **structured handoff** — a locked director card from `h3-short-drama-director`, or a `minimax-h3` brief — read `references/comfy-handoff.md`, lock `prompt_mode` from it, and apply its override table. Do not re-guess the mode from the images alone.
...
- **皮肤真实感锁（任何人出现的镜头必写，写进正文；H3 无独立负面提示词栏，必须正向点名）**：matte realistic skin with visible pores and fine texture, `completely free of oily shine, greasy glow, waxy highlights or plastic smoothing`；脸部特写另加 `absolutely no harsh specular hot spots`。光线优先用柔光写法（soft diffused daylight / window light / soft diffused overhead light with gentle falloff + subtle bounce fill），并点明额头、鼻梁、颧骨在光下保持哑光。参考图本身也必须来自写实摄影质感（见 `character-three-view`），CG 感参考图会锁死油腻观感。
