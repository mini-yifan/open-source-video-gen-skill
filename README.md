...
> 十几秒的单条视频，点一个技能直接出片，不走流程；成套的作品，交给两套人机协作视频模板——写方案、写剧本、做美术设定、导戏、开服务器、生成视频、配乐、混音、验收，你只在关卡上说「通过」或「改这里」。
>
...
- **零视频制作门槛。** 你不需要懂分镜、运镜、景别、混音。十秒的单条视频，你给画面感，AI 补全成专业提示词；成套的作品，AI 把它补全成完整的导演方案、镜头卡和提示词，并把方案写成文档请你过目。人的想象力负责天马行空，Agent 负责专业细节。
...
| `short-drama-production` 短剧模板 | 竖屏短剧（多集续作）；宣传片、软件演示、创意短片同用此流程 | 四个确认关卡：Brief → 分集概要 → 剧本+时间轴 → 美术，成片后再定修改方向 | [短剧创作指南](./docs/短剧创作指南.md) |
...
| `h3-short-drama-director` | 导演：整集导演方案、5–15 秒镜头卡、连续性台账、样片裁决 |
...
- **只做一条 10 秒视频，也要装全部 11 个技能吗？** 不用。`minimax-h3` + `h3-prompt-writing` 就是最小可用组合；美女时尚单片直接 `beauty-video-gen`。全套装上也无妨——技能按需点将，用不到的不会被唤醒。
