...
> A clip that's a dozen seconds long takes a single skill and zero pipeline; finished, multi-part works get one of two human-AI video templates — treatment, script, art bible, directing, server power-up, generation, scoring, mixing, and review. You only say "approved" or "change this" at the gates.
>
...
- **Zero video-production background required.** You describe the vibe and the image. For a single clip, the AI expands it into a professional prompt; for a finished work, it expands it into a full directing treatment, shot cards, and model prompts, then writes it all down and asks for your sign-off. Humans supply imagination; the agent supplies the craft.
...
| `short-drama-production` — short drama | vertical short drama (multi-episode); the same pipeline also runs product promos, software demos, and creative shorts | four approval gates: brief → series outline → script + timeline → art bible; you set the revision direction after delivery | [短剧创作指南](./docs/短剧创作指南.md) |
...
## Eleven skills, hired à la carte
...
**The screenwriter and director, who only appear inside the drama template:**

| Skill | What it does |
|---|---|
| `short-drama-screenplay-writing` | screenwriter: scene design, playable scripts, character-specific dialogue, timelines |
| `h3-short-drama-director` | director: episode treatment, 5–15 s shot cards, continuity ledger, take review |
...
- **Do I need all 11 skills for one 10-second clip?** No. `minimax-h3` + `h3-prompt-writing` is the minimal viable set; for a fashion clip call `beauty-video-gen` directly. Installing everything is fine too — skills are hired on demand, and the ones you don't need never wake up.
